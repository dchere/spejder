...
- **One-time seed** (`bad_cloud.ensure_bad_cloud_initialized`): shared skill hygiene (`run_skill_hygiene_stages`) after blocked DB cleanup on GUI background sync and CLI `process-inbox`; seeds cloud from existing `blocked_skills` when `bad_cloud_seeded` is false, prunes only entries **not** in the seeded batch. Threshold calibration is separate (see below).
- **Threshold recalibration** (`bad_cloud.recalibrate_and_store_threshold`): single write path — runs on every GUI background sync and `process-inbox` via `run_skill_hygiene_stages` after cloud init. Uses p95 of **mature** non-blocked enabled `skill_patterns` (`created_at` older than 1 day; falls back to all eligible non-blocked patterns if none are mature) vs p50 of `blocked_skills`, with `skill_bigram_threshold_margin` and floor `0.1`. Persists result to `skill_bigram_toxicity_threshold` (cache for extraction between syncs). Empty cloud → `inf` / clears stored threshold.
