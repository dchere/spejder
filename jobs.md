...
**Merge order in `core.extract_job_entries`:** enabled artifacts (by priority) produce a by-link map first; both the text-seed loop and the new-link loop call `merge_entry_fields` in `parsing/merge.py` (first non-empty value per `_ENTRY_FIELD_KEYS` wins; later maps never overwrite). Platform order is Google → The Hub → Djinni → Danfoss → Vestas → Novo Nordisk → Oracle CX → Demant → Jobindex (with `work_type` / `source` stripped before merge) → generic HTML. Text-seed entries copy non-empty merged platform+HTML fields on top (platforms beat text; platforms no longer last-wins across each other). Artifact fields then fill remaining empties via `_fill_empty_fields`; `_work_type_from_html_for_link` runs after merge when `work_type` is still empty; `_provider_from_link` is the fallback source. Artifact-only links (no built-in/`_is_job_link` hit) are appended after the links loop.
