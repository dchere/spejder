...
- `get_job_skills(db_path, job_id)` — raw cached skill names for one job (unfiltered; use `extractors.skill_extractor.get_job_skills_filtered` for display/scoring)
- `get_job_skills_for_jobs(db_path, job_ids)` — batch variant; returns `{job_id: [names]}` in one query (unfiltered)
