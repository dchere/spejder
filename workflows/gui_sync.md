...
7. Optionally run async antipattern sync (daemon thread). Skipped runs log `skip_reason` and do **not** reload the profile or queue a dashboard rebuild; successful commits do both.

**Constraints:**
- No wildcard imports (`import *`); keep imports explicit.
