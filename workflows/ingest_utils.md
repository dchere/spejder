...
- `quarantine_unparsed_inbox_files(ingest_stats, *, inbox_root="", quarantine_dir="")` moves `found=0` / unparsed-status files into quarantine with a `.json` sidecar (`status`, `quality`, `synth_reason`, …).
- `log_ingest_parse_outcomes(sync_log, ingest_stats)` writes one `event=parse_file` sync-log line per non-`ok` file.
- `print_ingest_file_stats(ingest_stats)` logs per-file ingest counters with truncation (includes `weak_dropped` / `status` / `quality` / `synth_reason` when present).
